...
| Vard | Dashboard, Reporting & Landing Page | `tasks-vard.md` |
...
### API Contract: Shivam → Vard
Shivam's `POST /api/sessions/{id}/finish` and `GET /api/sessions/{id}/report` feed Vard's report UI.
...
Shivam and Vard read from the tables Ishaq creates — never write migrations themselves.
...
| After Shivam Task 2.4 | `/finish` endpoint is live — Vard can build report UI | Shivam → Vard |
